...
void handle_update_team_names(SOCKET client_socket, const char* buffer) {
    // Find Authorization header
    char* auth_header = strstr(buffer, "Authorization: Basic ");
    if (!auth_header) {
        send_error_response(client_socket, "Unauthorized access");
    ...
    if (len > 0 && len < 256) {
        ...
        if (strcmp(base64_creds, "YWRtaW46YWRtaW4xMjM=") == 0) {
            ...
            if (body) {
                ...
                // Parse JSON to get team names
                char* team1_name = strstr(body, "\"team1Name\":\"");
                char* team2_name = strstr(body, "\"team2Name\":\"");
                
                if (team1_name && team2_name) {
                    team1_name += 12;  // Skip "team1Name":"
                    team2_name += 12;  // Skip "team2Name":"
                    
                    // Extract team names
                    char* end1 = strchr(team1_name, '"');
                    char* end2 = strchr(team2_name, '"');
                    ...
                    if (end1 && end2) {
                        ...
                        *end2 = '\0';
                        ...
                        // Update team names
                        strncpy(team1.name, team1_name, sizeof(team1.name) - 1);
                        strncpy(team2.name, team2_name, sizeof(team2.name) - 1);
...
int main() {
    ...
    while (1) {
        ...
        else if (strncmp(buffer, "POST /api/team-names", 20) == 0) {
            handle_update_team_names(client_socket, buffer);
        }
